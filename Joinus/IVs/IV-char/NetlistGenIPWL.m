%Function that gets the netlist from OrgNetlist and breaks it down for manipulation of it.
...
function [ReturnVal , TimeStep] = NetlistGenIPWL(OrgNetlist,NewNetlist,ParamName,Imin,Imax,Points,Tstep,WaitTime,AddNoise)

KBolt = 1.38e-23;
temperature=4.2;
Istep=(Imax-Imin)/Points; %In mA
...
for LineCNTR=1:Points
 ParamVal=['PWL(0ps 0mA ', num2str((Tstep+WaitTime)*LineCNTR), 'ps 0mA ', num2str((Tstep+WaitTime)*LineCNTR+Tstep), 'ps ', num2str(Istep),'mA)'];   
 Newlines{LineCNTR}=['Inew',num2str(LineCNTR),'   ',aa(2,:),' ',aa(3,:),' ',ParamVal];
end
for LineCNTR=1+Points:Points*2
 ParamVal=['PWL(0ps 0mA ', num2str((Tstep+WaitTime)*LineCNTR), 'ps 0mA ', num2str((Tstep+WaitTime)*LineCNTR+Tstep), 'ps ', num2str(-1*Istep),'mA)']; 
 Newlines{LineCNTR}=['Inew',num2str(LineCNTR),'   ',aa(2,:),' ',aa(3,:),' ',ParamVal];
end
...
tlines{TranIndex}=[bb(1,:),' ',bb(2,:),' ', num2str((Tstep+WaitTime)*(LineCNTR+1)),'PS ',bb(4,:),' ',bb(5,:)];
